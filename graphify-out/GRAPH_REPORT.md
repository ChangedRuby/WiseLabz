...
## Corpus Check
- 506 files · ~622,395 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2873 nodes · 6318 edges · 96 communities detected
- Extraction: 54% EXTRACTED · 45% INFERRED · 0% AMBIGUOUS · INFERRED: 2827 edges (avg confidence: 0.8)
...
1. `Errorf()` - 397 edges
2. `newTestApp()` - 193 edges
3. `Store` - 179 edges
4. `JSON()` - 122 edges
5. `New()` - 116 edges
6. `contains()` - 96 edges
7. `newDocTestStore()` - 94 edges
8. `Error()` - 85 edges
...
- `open()` --calls--> `newEngineTestStore()`  [INFERRED]
  web/src/features/services/ServiceDetailPage.tsx → backend/internal/doc/engine_test.go
- `open()` --calls--> `newDocTestStore()`  [INFERRED]
  web/src/features/services/ServiceDetailPage.tsx → backend/internal/store/doc_test.go
...
Cohesion: 0.02
Nodes (79): Handler, Config, NewRouter(), spaHandler(), wsRoleLabel(), Handler, newToken(), sanitize() (+71 more)

### Community 1 - "Community 1"
Cohesion: 0.01
Nodes (115): claudeProvider, ollamaEmbedder, openAICompatibleProvider, openAIEmbedder, TestTemplatesConcurrentUpdatesCreateDistinctVersions(), TestConcurrentIssuePairUniqueTokenIDs(), AuthError, ConfigField (+107 more)
...
Cohesion: 0.02
Nodes (45): join(), Errorf(), attentionSeverityRank(), AttentionItem, actorRoleLabel(), auditFilterClause(), scanAuditRecord(), scanAuditRecordRows() (+37 more)

### Community 4 - "Community 4"
Cohesion: 0.03
Nodes (185): bulkSnoozeItemResult, bulkSnoozeRequest, NewHandler(), TestBulkSnooze(), TestDismissNotFound(), TestGetNotFound(), TestListEmpty(), TestResolveNotFound() (+177 more)

### Community 5 - "Community 5"
Cohesion: 0.03
Nodes (100): RegisterClaude(), TestClaudeSuggestErrors(), TestRegisterClaudeDefaults(), RegisterOllamaEmbedder(), RegisterOpenAIEmbedder(), Embedder, NewEmbedRegistry(), RegisterOpenAICompatible() (+92 more)

### Community 6 - "Community 6"
Cohesion: 0.04
Nodes (94): TestAPIKeyLifecycle(), TestAPIKeyNotFound(), TestLookupAPIKeyReflectsLiveRole(), TestLookupAPIKeyRejectsDisabledUser(), TestRevokeAllAPIKeysForUser(), TestTouchAPIKeyLastUsed(), TestCreateAuditRecordAndListFiltering(), TestListAllAuditRecords() (+86 more)

### Community 7 - "Community 7"
Cohesion: 0.04
Nodes (63): EmbedRegistry, TestAllConnectorImplementationsRegister(), RedactConnectorConfig(), Handler, AttributeSpec, ConfigValidationError, Factory, AttributeCatalog() (+55 more)

### Community 8 - "Community 8"
Cohesion: 0.06
Nodes (54): RequestedFields(), Register(), newEngineTestStore(), seedEngineConnector(), seedEngineTemplate(), TestGenerateFromSnapshotIncludesDependencies(), TestGenerateFromTemplateReturnsVersionPersistenceError(), TestGenerateFromTemplateStillPersists() (+46 more)

### Community 9 - "Community 9"
Cohesion: 0.05
Nodes (51): TestAuthMiddlewareAcceptsNonAdminAPIKey(), TestAuthMiddlewareAPIKeyLifecycle(), TestAuthMiddlewareRejectsExpiredAndRevokedAPIKeys(), APIKeyChecker, AuditRecorder, ConnectorRoleChecker, contextKey, elevationError (+43 more)
...
Cohesion: 0.06
Nodes (48): AIConfigSummary, connectorIDs(), docIDs(), Export(), exportDocs(), exportTemplates(), ExportToFile(), exportWithin() (+40 more)
...
Cohesion: 0.08
Nodes (26): registerFailThenSucceed(), TestIsRetryable(), TestSuggestWithFallbackAdvancesOnRetryableError(), TestSuggestWithFallbackAllFail(), TestSuggestWithFallbackFirstProviderSucceeds(), TestSuggestWithFallbackNoProviders(), TestSuggestWithFallbackStopsOnNonRetryableError(), TestOpenAICompatibleSuggestErrors() (+18 more)
...
Cohesion: 0.1
Nodes (24): TestEmailDomainAllowed(), TestOIDCRoleForGroups(), clearOIDCFlowCookie(), emailDomainAllowed(), oidcFlowCookieName(), oidcRoleForGroups(), randomOIDCToken(), readOIDCFlowCookie() (+16 more)
...
Cohesion: 0.2
Nodes (2): TestFetchWithFieldsHintSkipsUnrequestedCalls(), TestValidateUsesTokenAndSurfacesStatus()
...
## Knowledge Gaps
- **193 isolated node(s):** `testAuditCall`, `contextKey`, `APIKeyChecker`, `UserStatusChecker`, `ConnectorRoleChecker` (+188 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 40`** (10 nodes): `proxmox_test.go`, `TestDoRequestContextTimeout()`, `TestDoRequestErrorCases()`, `TestFetchIncludesHostAndStorageDependencies()`, `TestFetchSurfacesMalformedNodesResponse()`, `TestFetchWithFieldsHintSkipsUnrequestedCalls()`, `TestRestart()`, `TestStartStop()`, `TestValidateUsesTokenAndSurfacesStatus()`, `TestWritableFields()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 45`** (8 nodes): `scheduler.go`, `Runner`, `.AddJob()`, `.EntryCount()`, `.jobContext()`, `.RemoveJob()`, `.Start()`, `.Stop()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 50`** (6 nodes): `eventLabel()`, `findRoute()`, `rowSeverity()`, `setCell()`, `setRowSeverity()`, `EventRoutingTable.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 131`** (2 nodes): `Branch Naming Convention`, `Pull Request Process`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 132`** (2 nodes): `chi HTTP Router`, `gorilla/websocket`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 133`** (2 nodes): `viper Config Loader`, `WISELABZ_ Env Var Config Override`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **What is the exact relationship between `Topbar Notification Center (deferred from V1)` and `NotificationDelivery Schema (per-channel delivery/retry)`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `Errorf()` connect `Community 3` to `Community 0`, `Community 1`, `Community 34`, `Community 5`, `Community 7`, `Community 8`, `Community 10`, `Community 11`, `Community 45`, `Community 14`, `Community 15`, `Community 16`, `Community 17`, `Community 18`, `Community 25`, `Community 26`?**
  _High betweenness centrality (0.213) - this node is a cross-community bridge._
- **Why does `newTestApp()` connect `Community 2` to `Community 0`, `Community 1`, `Community 5`, `Community 8`, `Community 9`?**
  _High betweenness centrality (0.135) - this node is a cross-community bridge._
- **Why does `RunMigrations()` connect `Community 5` to `Community 2`, `Community 3`, `Community 4`, `Community 6`, `Community 8`, `Community 11`, `Community 15`?**
...
- **Are the 184 inferred relationships involving `newTestApp()` (e.g. with `TestAlertsListSuccess()` and `TestAlertsResolveRoleBoundary()`) actually correct?**
  _`newTestApp()` has 184 INFERRED edges - model-reasoned connections that need verification._