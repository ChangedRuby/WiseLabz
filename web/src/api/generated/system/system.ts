...
/**
 * Available at the server root; reports whether the HTTP server is running.
...
export const getHealthz = (
  ...
  return customInstance<Liveness>({ url: `/healthz`, method: 'GET', signal }, options);
};
...
/**
 * Available at the server root; returns 503 when the database is unavailable or migrations are pending.
...
export const getReadyz = (
  ...
  return customInstance<Readiness>({ url: `/readyz`, method: 'GET', signal }, options);
};
...
export const getGetReadyzQueryOptions = <
  ...
  TError = ErrorType<Readiness>,
>(options?: {
  query?: Partial<UseQueryOptions<Awaited<ReturnType<typeof getReadyz>>, TError, TData>>;
  request?: SecondParameter<typeof customInstance>;
}) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};
  ...
  const queryFn: QueryFunction<Awaited<ReturnType<typeof getReadyz>>> = ({ signal }) =>
    getReadyz(requestOptions, signal);
...
export type GetReadyzQueryError = ErrorType<Readiness>;
