...
--
-- Note: we build the replacement under a fresh name and rename *it* into
-- place, rather than renaming "connectors" itself. SQLite's ALTER TABLE
-- RENAME auto-rewrites every other table's foreign key clauses that
-- reference the renamed table's old name — renaming "connectors" away
-- would repoint service_snapshots/changes/docs/quality_findings/etc. at
-- "connectors_old" and they'd stay pointed there even after it's dropped.
-- Renaming an unreferenced table in at the end triggers no such rewrite.
